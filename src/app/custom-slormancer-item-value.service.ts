...
import { bankerRound, ComputedEffectValue, EffectValueSynergy, EffectValueUpgradeType, EffectValueVariable, list, MinMax, Rarity, round, valueOrDefault } from 'slormancer-api';

@Injectable()
export class CustomSlormancerItemValueService {
    ...
    private getComputedBaseValue(level: number, score: number, percent: boolean): number {
        return percent
            ...
            : score * (100 + (((level * 30 - 1.2) + level * level * 1.2))) / 100;
    }
    ...
    private computeAffixValue(level: number, reinforcment: number, score: number, value: number, percent: boolean, pure: number | null): number {
        ...
        if (valueRatio === 100) {
            console.log('level : ', level, 'score: ', score, 'baseValue : ', baseValue, 'valueRatio : ', valueRatio);
        }
