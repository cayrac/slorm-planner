...
import { compareString, enumValues, valueOrDefault } from '../../slormancer/util/utils';
...
export class ItemFormService {
    ...
    public initStatOptionsCache() {
        const stats = this.slormancerDataService.getGameDataStats();
        const bases = enumValues(EquipableItemBase);
    ...
    public getStatsOptions(base: EquipableItemBase, rarity: Rarity): Array<SelectOption<string>> {
        ...
        const baseKey = this.getBaseKey(base);
        const baseStats = this.STATS_OPTIONS_CACHE[baseKey]
        if (baseStats) {
