...
export class SlormancerActivableService {
    ...
    private parseSkillEffectValues(data: GameDataSkill): Array<AbstractEffectValue> {
        const valueBases = splitFloatData(data.DESC_VALUE_BASE);
    ...
    private applyActivableOverride(activable: Activable, ref: number) {
    ...
    public getHeroSkill(gameData: GameDataSkill): Activable {
        return {
            name: gameData.EN_NAME,
            icon: 'skill/' + gameData.REF,
            description: this.slormancerTemplateService.getSkillDescriptionTemplate(gameData),
            baseCooldown: gameData.COOLDOWN,
            cooldown: gameData.COOLDOWN / 60,
            baseCost: gameData.COST,
            cost: gameData.COST,
            costType: this.parseCostType(gameData.COST_TYPE),
            damageTypes: splitData(gameData.DMG_TYPE, ','),
            genres: splitData(gameData.GENRE, ',').filter(this.isGenre),
            values: this.parseSkillEffectValues(gameData),
        };
    }
}